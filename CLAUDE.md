...
## 뷰 전용 모드 / 외부 공유용 데모 모드
- `?view=1`: 완전 읽기 전용 모드 — 조회/검색/필터/미리보기/내보내기만 가능하고, 새 템플릿 등록·수정·저장·삭제·이미지 업로드/삭제·버튼 추가/삭제는 UI 자체가 숨겨지거나 비활성화됨. 같은 `index.html`, 같은 데이터소스를 그대로 사용 — 쿼리 파라미터로만 분기
  - **주의**: 이건 화면단 제한일 뿐, meatbox.co.kr 계정으로 로그인된 내부 사용자가 `?view=1`을 개발자도구로 우회하거나 URL에서 떼면 그대로 수정 가능함
- `?demo=1`: 실제 구글 시트/Apps Script에 **전혀 연결하지 않고** `seedData()`의 샘플 템플릿만 보여줌 (`isRemoteConfigured()`가 항상 false를 반환하도록 처리). 저장/삭제/이미지 업로드 등 백엔드를 건드리는 모든 경로가 코드 레벨에서 원천 차단됨
- **외주 개발자 등 외부 인력에게 화면만 보여줄 때는 `?view=1&demo=1`을 함께 사용할 것** — 실제 템플릿 데이터에 전혀 접근하지 않으면서 UI/기능 구조만 보여줄 수 있음
- 참고: 외부 인력이 meatbox.co.kr 구글 계정이 없다면, 애초에 Apps Script 배포 자체가 조직 계정으로 제한돼 있어 실제 데이터 API 호출도 불가능함(진짜 보안 경계는 이쪽). `?demo=1`은 그와 별개로 "링크를 열었을 때 항상 안전하게 샘플만 보이도록" 보장하는 장치
